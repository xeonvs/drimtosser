...
Public Class clsINI
    ...
    Public Property FileName() As String
        ...
        Set(ByVal Value As String)
            ...
            ' Файл в наличии?
            If Not My.Computer.FileSystem.FileExists(mFilename) Then
                ' Если нет, то...
                Exit Property
            End If

            ' Считываем файл
            sr = My.Computer.FileSystem.OpenTextFileReader(mFilename)
...
skip_line:
    ...
    Public Function UpdateFile() As Boolean
        ...
        Dim i, a As Integer
        Dim sw As StreamWriter = My.Computer.FileSystem.OpenTextFileWriter(mFilename, False)
        ...
        For i = 1 To UBound(AllSections)
            ...
            With AllSections(i)
                ...
                For a = 1 To UBound(.Values)
                    sw.WriteLine(.Values(a).Name & "=" & .Values(a).cntValue)
                Next a
            End With
            sw.WriteLine()
        Next i
